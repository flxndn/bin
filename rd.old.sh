...
#------------------------------------------------------------------------------- 
	...
	while getopts "hfg:0sev8a:" OPTION; do
		case $OPTION in
			...
			s) cat $CONFFILE;exit;;
			e) vi $CONFFILE;exit;;
			v) readonly VERBOSE=1; set -x; shift;;
	...
	if [ -z "$1" ] ; then
		...
		[ $VERBOSE -eq 1 ] && echo "$PROGNAME $COLORES $GEOMETRY -x 0x80 $OPCION_CONSOLA $host"
		...
		host=$(grep "^$1	" $CONFFILE|cut -f2-)
		[ $VERBOSE -eq 1 ] && echo "rdesktop $COLORES $GEOMETRY -x 0x80 $OPCION_CONSOLA $host"
		#rdesktop $COLORES $GEOMETRY -x 0x80 $OPCION_CONSOLA $host &
		rdesktop $COLORES $GEOMETRY -x 0x80 $OPCION_CONSOLA $host
	fi 
}
#------------------------------------------------------------------------------- 
